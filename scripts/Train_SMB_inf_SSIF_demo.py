...
notes = [
    ...
    'Settings:.'
    '- training: Stateful mini-batch (SMB) algorithm.'
    '- Model: one layer GRU.'
    ]
...
# Remove the first sample since it could not match the order.
num_train_samples_stateful = num_train_samples - 1
# Generate the samples' index, and exclude the first sample since it does not align well with later samples.
idx = np.arange(num_train_samples_stateful)
# Generate the samples index for RMB learning, this idx_MB includes the first few sequences.
idx_MB = idx[:batch_size-1].reshape(batch_size-1,-1).T
# Generate the samples index for SMB learning
idx_stateful = idx[num_train_samples_stateful % batch_size:].reshape(64,-1).T
# Generate the indexes of SMB samples
train_idx_arr_stateful = train_idx_arr[1:]
# Generate the indexes RMB samples
train_idx_arr_MB = np.vstack((train_idx_arr[0], train_idx_arr_stateful[idx_MB[0]]))

# Generate the stateful samples
x_train_sp_stateful = x_train_sp[1:]
y_train_sp_stateful = y_train_sp[1:]
m_train_sp_stateful = m_train_sp[1:]
# # Generate the MB samples
# x_train_sp_MB = np.vstack((x_train_sp[0], x_train_sp_stateful[idx_MB[0]]))
...
for epoch in range(epochs):
    ...
    model1.train()
    for j in range(0,len(idx_MB)+len(idx_stateful)):
        # Sets the gradients of all optimized torch.Tensors to zero.
        model1.zero_grad()
        if j < len(idx_MB):
            batch_x = torch.cat((x_train_sp[0][None,:,:], x_train_sp_stateful[idx_MB[0]]),0)
            batch_y = torch.cat((y_train_sp[0][None,:,:], y_train_sp_stateful[idx_MB[0]]),0)
            batch_m = torch.cat((m_train_sp[0][None,:,:], m_train_sp_stateful[idx_MB[0]]),0)
        else:
            batch_x = x_train_sp_stateful[idx_stateful[j-len(idx_MB)]]
            batch_y = y_train_sp_stateful[idx_stateful[j-len(idx_MB)]]
            batch_m = m_train_sp_stateful[idx_stateful[j-len(idx_MB)]]
        if j < len(idx_MB)+1:
            hidden_head = model1.init_hidden(batch_size)
        else:
            # Assert the sequences between minibatches follow the sequential order correctly.
            assert(train_idx_arr_stateful[idx_stateful[j-len(idx_MB)]][-1,-1] - train_idx_arr_stateful[idx_stateful[j-len(idx_MB)-1]][-1,-1] == n_steps)
            # Allow the hidden states to pass between minibatch.
            hidden_head = hidden_tail
            
        pred,hiddens = model1(batch_x,hidden_head)
        # Multi-task losses
        # batch_losses = torch.zeros(n_classes)
        # for k in range(n_classes):
        #     batch_losses[k] = MSE(pred[:,:,k], batch_y[:,:,k], batch_m[:,:,k])
        # batch_loss = torch.sum(batch_losses)
        batch_loss = MSE(pred, batch_y, batch_m)
        hiddens.detach_()
        batch_loss.backward()
        optimizer.step()
        with torch.no_grad():
            hidden_tail = hiddens[:,-1,:].detach()[None,:,:]
    scheduler.step()
    endtime=time.time()
