...
for epoch in range(epochs):
    ...
    model1.train()
    # Shuffling
    idx=torch.randperm(x_train_sp.size()[0]).to(device) 
    for j in range(0,num_train_samples,batch_size):
        ...
        hidden_head = model1.init_hidden(ej - sj)
    ...
    model1.eval()
    with torch.no_grad():
        hidden_head = model1.init_hidden(num_train_samples)
        pred_train,hiddens = model1(x_train_sp,hidden_head)
        hidden_head = model1.init_hidden(num_valid_samples)
        pred_valid,hiddens = model1(x_valid_sp,hidden_head)
        hidden_head = model1.init_hidden(num_test_samples)
        pred_test,hiddens = model1(x_test_sp,hidden_head)
        epoch_loss_train.append(MSE(pred_train, y_train_sp, m_train_sp).cpu().numpy())
        epoch_loss_valid.append(MSE(pred_valid, y_valid_sp, m_valid_sp).cpu().numpy())
        epoch_loss_test.append(MSE(pred_test, y_test_sp, m_test_sp).cpu().numpy())
        ...
        if epoch_loss_valid[epoch] < loss_val_best:
            ...
            torch.save({'epoch': epoch,
                    ...
                    'los_val': epoch_loss_valid[epoch],
                    }, path_save)
